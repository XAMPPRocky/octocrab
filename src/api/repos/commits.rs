...
#[derive(serde::Serialize)]
pub struct RepoCompareCommitsBuilder<'octo, 'r> {
    #[serde(skip)]
    handler: &'r RepoHandler<'octo>,
    #[serde(skip)]
    basehead: String,
...
impl<'octo, 'r> RepoCompareCommitsBuilder<'octo, 'r> {
    pub(crate) fn new(handler: &'r RepoHandler<'octo>, basehead: String) -> Self {
        Self {
            handler,
            basehead,
            per_page: None,
    ...
    /// Sends the actual request.
    pub async fn send(self) -> crate::Result<crate::models::commits::CommitComparison> {
        let route = format!("/{}/compare/{}", self.handler.repo, self.basehead);
        self.handler.crab.get(route, Some(&self)).await
    }
}
