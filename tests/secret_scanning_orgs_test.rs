...
async fn setup_org_secrets_api(template: ResponseTemplate) -> MockServer {
    let mock_server = MockServer::start().await;

    Mock::given(method("GET"))
        .and(path(format!("/orgs/{ORG}/secret-scanning/alerts")))
        .respond_with(template.clone())
        .mount(&mock_server)
        .await;
    setup_error_handler(
        &mock_server,
        &format!("GET on /orgs/{ORG}/secret-scanning/alerts was not received"),
    )
    .await;
...
#[tokio::test]
async fn check_org_secrets_alert_list_200() {
    ...
    let mock_server = setup_org_secrets_api(template).await;
...
#[tokio::test]
async fn check_org_secrets_alert_query_params() {
    let s: &str = include_str!("resources/check_org_secrets_alerts.json");
    let alert: Vec<SecretScanningAlert> = serde_json::from_str(s).unwrap();
    let template = ResponseTemplate::new(200).set_body_json(&alert);
    let mock_server = MockServer::start().await;
